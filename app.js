...
function table(headers,rows,className=''){return `<section class="panel table-panel"><div class="table-head"><h3>Chi tiết</h3><span class="panel-note">Cuộn ngang để xem thêm</span></div><div class="table-scroll"><table class="${className}"><thead><tr>${headers.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.join('')}</tbody></table></div></section>`}
...
function renderCombo(){
 ...
 const programs={
  ...
  comboHpcTocs:{label:'Combo HPC'},
  comboHpcChante:{label:'HPC Chanté Active'},
  comboCanuTocs:{label:'Combo Canu'},
  comboCoreTocs:{label:'Ngành core'},
  newOrder200k:{label:'Đơn mở mới 200k'}
 };
 ...
 `<div class="cards combo-cards">${metricCard('KH ĐANG HIỂN THỊ',fmtNumber(rows.length),state.comboProgram==='all'?'Tất cả khách hàng Team SS Pháp':programs[state.comboProgram].label,null,'◎')}${metricCard('COMBO HPC',fmtNumber(stats[0].count),'Khách hàng được áp dụng',stats[0].count/(allRows.length||1),'H')}${metricCard('HPC CHANTÉ ACTIVE',fmtNumber(stats[1].count),'Khách hàng được áp dụng',stats[1].count/(allRows.length||1),'C')}${metricCard('ĐƠN MỞ MỚI 200K',fmtNumber(stats[4].count),'Khách hàng được áp dụng',stats[4].count/(allRows.length||1),'₫')}</div>`+
 `<section class="panel combo-analysis"><div class="panel-head"><div><h3>Phân bổ theo từng chương trình Combo</h3><span class="panel-note">Số khách hàng hiển thị trực tiếp trên từng cột</span></div><span class="badge">NPP Tâm Hạnh</span></div><div class="combo-program-list">${stats.map(item=>`<button data-combo-program="${item.key}" class="combo-program-row"><span>${esc(item.label)}</span><div class="bar combo-bar"><i style="width:${item.count/maxCount*100}%"></i><small>${fmtNumber(item.count)} KH</small></div><strong>${fmtNumber(item.count)} KH</strong></button>`).join('')}</div></section>`+
 table(['Mã KH','Tên KH','Địa chỉ','Phường / Xã','Thứ','DDKD','HPC','HPC Chanté','Canu','Core','Chi tiết core','Mở mới 200k'],rows.map(d=>`<tr><td title="${esc(d.customerCode)}">${esc(d.customerCode)}</td><td class="person" title="${esc(d.customer)}">${esc(d.customer)}</td><td class="muted" title="${esc(d.address)}">${esc(d.address)}</td><td title="${esc(d.ward)}">${esc(d.ward)}</td><td>${esc(d.routeDay)}</td><td title="${esc(d.rep)}">${esc(d.rep)}</td><td>${yes(d.comboHpcTocs)}</td><td>${yes(d.comboHpcChante)}</td><td>${yes(d.comboCanuTocs)}</td><td>${yes(d.comboCoreTocs)}</td><td class="muted" title="${esc(d.coreDetail)}">${esc(d.coreDetail)}</td><td>${yes(d.newOrder200k)}</td></tr>`),'combo-table');
